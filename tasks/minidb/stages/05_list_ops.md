...
1. **CONTAINS operator**: `expr CONTAINS value`
   - Returns `true` if `expr` evaluates to a list containing `value`.
   - Comparison uses the same coercion rules: `[1, 2, 3] CONTAINS '2'` is `true` (string '2' coerces to 2).
...
2. **FLATTEN**: `SELECT cols, FLATTEN(listcol) AS alias FROM table`
...
3. **Nested lists**: `FLATTEN` only expands one level. `FLATTEN([1, [2, 3]])` produces rows `1` and `[2, 3]`.
